...
_Created: 12-06-2026 · Last updated: 01-09-2026_

> **~~Human-gate 27-08-2026~~ — СНЯТ 01-09-2026** ([H3777](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3777-Opus_RussianRamayana_dh-roadmap-block-diagnose-drain_31.08.26.md), Opus 5 `claude-opus-5`).
> Врезка требовала «fetch MP3 of books V and VI from Yandex Disk». Выкачивать нечего:
> на этой машине лежат **все 7 книг**, и все 7 сходятся по SHA-256 с
> [data/audio.json](https://github.com/gasyoun/RussianRamayana/blob/main/data/audio.json).
> Ни один файл не скачивался — сделан только замер уже лежащего.
> Проверить в один шаг: `python scripts/audio_inventory.py --verify` (в **главном** чекауте,
> не в worktree). Корень двух ложных «отсутствий» — раздел «Разрешение противоречия по MP3»
> ниже. Оставшиеся гейты Фазы 1 — права наследников Потаповой и аккаунт archive.org, и они
> к Яндекс Диску отношения не имеют.
...
- [x] **(закрыт замером 01-09-2026, [H3777](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3777-Opus_RussianRamayana_dh-roadmap-block-diagnose-drain_31.08.26.md))** Свести воедино MP3 всех 7 книг. Сводить не пришлось: на Windows-машине (главный чекаут `C:\Users\user\Documents\GitHub\RussianRamayana`) лежат **7 из 7** файлов, и у всех семи совпадают и `size_bytes`, и `sha256` с [data/audio.json](https://github.com/gasyoun/RussianRamayana/blob/main/data/audio.json) и с таблицей [docs/ia-upload.md](https://github.com/gasyoun/RussianRamayana/blob/main/docs/ia-upload.md). Медиа не выкачивалось — замерено уже лежащее. Воспроизводится: `python scripts/audio_inventory.py --verify` → `OK — all 7 books present and SHA-256 matched`. Оба прежних «отсутствия» объяснены в разделе «Разрешение противоречия по MP3» ниже; Яндекс Диск для аудио не нужен и никогда не был нужен.
...
| 7 | Фаза 1: «архив неполон», MP3 V–VI брать с Яндекс Диска | ~~Противоречие не разрешается агентом~~ → **разрешено 01-09-2026 замером**: все 7 книг на месте, 7/7 по SHA-256. Разбор — раздел ниже |
...
## Разрешение противоречия по MP3 (01-09-2026, H3777)

Один и тот же чек-бокс Фазы 1 трижды получал разный ответ. Замер 01-09-2026 на
Windows-машине даёт **7 из 7 книг, все с совпадающим SHA-256**; ниже — почему два
предыдущих замера дали «пять» и почему ни один из них не был ошибкой измерения.

| Дата | Где мерили | Результат | Что это было |
|---|---|---|---|
| 12-06-2026 | Windows, главный чекаут | 7 из 7 | Верно. Записано в [.ai_state.md](https://github.com/gasyoun/RussianRamayana/blob/main/.ai_state.md): «MP3 всех 7 книг ЕСТЬ локально» |
| 26-08-2026 (H3003) | клон/worktree | 5 из 7 (нет V–VI) | **Артефакт worktree.** Книги V (124 MB) и VI (154 MB) — в `.gitignore` (лимит GitHub 100 MiB), пять остальных **в git**. Связанный git-worktree не переносит игнорируемые файлы, поэтому там физически видны ровно пять — независимо от того, что лежит на машине |
| 28-08-2026 (A05) | **macOS**, полный обход `find` + Spotlight | 5 из 7 | **Верно для той машины.** Это другой компьютер; V–VI на него никогда не копировали |
| 01-09-2026 (H3777) | Windows, главный чекаут | **7 из 7, SHA-256 7/7** | Верно. `python scripts/audio_inventory.py --verify` |

Вывод: противоречия между записями нет — есть **две машины и один worktree**, а
формулировка «архив неполон, взять с Яндекс Диска» неверна с самого начала.
Полный комплект держит Windows-машина; она же держит `bin/ffmpeg` и всю
подготовку загрузки на IA, так что Фазу 1 исполнять надо с неё.

**Как не повторить.** `scripts/audio_inventory.py --verify` печатает предупреждение,
когда запущен из связанного worktree (`.git` — файл, а не каталог), и прямо говорит,
что отсутствие там ничего не доказывает. Правило шире одного репозитория: перепись
**игнорируемого** ассета, сделанная в worktree, всегда покажет ложное отсутствие —
[Uprava/FINDINGS.md](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md).

Что после этого действительно держит Фазу 1 — ровно два человеческих акта, ни один
из которых не про файлы: **(1)** решение по правам наследников В. Потаповой
([docs/RIGHTS.md](https://github.com/gasyoun/RussianRamayana/blob/main/docs/RIGHTS.md),
«Открытые вопросы» № 1) и **(2)** логин в archive.org (`ia configure`) для создания
item — процедура целиком расписана в
[docs/ia-upload.md](https://github.com/gasyoun/RussianRamayana/blob/main/docs/ia-upload.md),
включая вариант `access-restricted-item:true`, который резервирует identifier, не
публикуя файлы.
