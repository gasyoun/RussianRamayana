...
сверяются с рабочим словарем в Excel
...
переносит разметку в верстку, строит таблицы указателя (`IndexList`),
...
- **Построение указателей Тома II** — тот же пайплайн (стадии 1–4) с нуля,
  так как таблицы `IndexList` для Тома II еще не построены; общий с Томом I
...
- **Пре-флайт-валидация словаря перед стадией `[1]`** — прогон
  ...
  по `.xlsx`: read-only отчет по **внутренним** дефектам рабочих листов (проза/комментарий
  ...
  Windows+Excel. Отчет по текущему словарю —
...
- **Обучение новой стадии/скрипту на игрушечном примере** — перед прогоном
  ...
  не рискуя реальной версткой тома.
...
- **Перенос пайплайна на будущие тома/издания серии «Литературные
  ...
  написаны достаточно обобщенно, чтобы применяться к другим тегированным
...
- **Пре-флайт-валидатор словаря** (кроссплатформенно, без InDesign/Excel):
  ...
  (в отчет-файл — `--report <файл.md>`; один лист — `--sheet "Именной"`). Требует
...
Из [`CLAUDE.md`](https://github.com/gasyoun/RussianRamayana/blob/main/Litpam-Indexator/CLAUDE.md) и `ForIndex.jsxinc` — жесткий контракт имен,
...
- Стадия `[2. Проверка правильности данных в таблицах]` для этого проекта
  в основном не нужна (нет совпадающих grep-запросов) — см. ее собственный
  `info.txt`.
- Скрипты стадий 1/3 намеренно отказываются работать при переполнении
  текстового фрейма или на несохраненном документе — это ожидаемое
  поведение, не баг.
...
- **Руководство закалено + процесс усилен (08-07-2026, H363).** В
  ...
  [`tools/validate_dictionary.py`](https://github.com/gasyoun/RussianRamayana/blob/main/Litpam-Indexator/tools/validate_dictionary.py)
  (43 находки на текущем словаре, отчет в
...
- Скрипты и документация — зрелые и завершенные (авторский инструментарий,
...
- Применение пайплайна к реальному тексту — только в начале: чек-лист
  ...
  работы.
- **Том II** к индексации еще не приступали — таблицы `IndexList` для него
  ...
  начатый.
- Готовые верстки в `InDesign/` датированы 12.10.25 — предшествуют
...
- В `git log` эта папка присутствует только в 3 последних коммитах
  ...
  свежее добавление, инструментарий сюда еще не «оброс» правками по месту.
