...
btnAndChkbx.alignChildren = ["fill", "fill"]; //
...
var complexReg = btnAndChkbx.add("checkbox");
complexReg.value = false;
complexReg.helpTip = "По умолчанию этот флажок сброшен. А если установлен, то для каждого слова термина устанавливается свой режим работы с регистром: если слово начинается со строчной буквы, то вариант (?i), если с прописной, то (?-i)";
...
for (var r = 0; r < rowsN; r++) { // r++    
    procTextInCell (mySel.rows[r], 0, onePadezh.value, complexReg.value);
...
for (var r = 0; r < rowsN; r++) { // r++
     procTextInCell (mySel.rows[r], 1, onePadezh.value, complexReg.value);
     if (r%stepToShow == 0) pBar.hit();    
    } // r++
pBar.close();
} // fWO
...
////
function procTextInCell (row, theAction, imPad, complexReg) { // procTextInCell
...
for (w = 0; w < wordsCount; w++) { // w++
  ...
  //  rw = getWord (words, imPad, usePunct.value, w, theAction);    
    if (useFamily) { // useFamily
        if (complexReg == false) wordsR[w] = rw; // дефис был исследован при определении какая фамилия -- двойная или одиночная
        else {
            firstChar = rw[2]; // это буква после \b
          //  $.writeln("1 firstChar = " + firstChar);
            try {
                if (capitals.match(firstChar) != null) rw = "(?-i)" + rw;
                else rw = "(?i)" + rw;
                }
            catch (e) { }
            wordsR[w] = rw;
        ...
        if (splitAr.length == 1 && splitAr2.length == 1) {
            if (complexReg == false) wordsR[w] = rw;
        else {
            firstChar = rw[2]; // это буква после \b
           // $.writeln("2 firstChar = " + firstChar);
            try {
                if (capitals.match(firstChar) != null) rw = "(?-i)" + rw;
                else rw = "(?i)" + rw;
                }
