...
| Прогон `UseReadyTable` затёр предыдущую таблицу | Автоинкремент имени закомментирован (26.11.2024) — выход всегда `IndexList-000.indd` | Пересохранять под нужным номером сразу после каждого прогона; фикс запланирован (H377) |
| Grep `^[dfhjmstwz]-` не убирает префиксы маркеров из собранного указателя | Дрейф контракта: `markLetters` сменились на `a…i`, а help-текст `AddMarker.jsx` и регекс очистки остались от старого набора | Использовать `^[a-i]-`; фикс help-текста запланирован (H377) |
| В указателе английское «see» вместо «см.» | `AddSeeTopic.v.3.v.3.jsx` берёт «см.» из `sets/#AddSeeTopic.ini`; без ini остаётся дефолт `textPointer="see"` из `ForIndex.jsxinc` | Один раз выбрать вариант «см.» в диалоге (создастся ini); дефолт-фикс запланирован (H377) |
...
> ⚠️ **Ловушка перезаписи (аудит кода 08-07-2026).** В текущей версии скрипта
> автоинкремент имени **закомментирован** (правка автора от 26.11.2024): скрипт всегда
> сохраняет результат под жёстким именем `IndexList-000.indd` — каждый прогон **затирает
> предыдущую таблицу**. (В видео 10 показано старое поведение с автонумерацией — оно
> больше не действует.) До возврата автоинкремента (план H377): сразу после прогона
> вручную пересохранить/переименовать файл в нужный номер `IndexList-001`…`004`, и только
> потом запускать следующий лист.
...
производительность; 🟡 — гигиена кода. План фиксов — H377.

- 🔴 **`UseReadyTable.v.7.jsx` — перезапись выходного файла.** Автоинкремент имени
  (`IndexList-001`, `-002`…) закомментирован 26.11.2024, вместо него жёсткое имя
  `IndexList-000.indd` — каждый прогон затирает предыдущую таблицу. (Прежняя редакция
  MANUAL утверждала «скрипт по умолчанию предлагает 001» — это описывало старое
  поведение из видео 10 и было неверно для текущего кода.)
- 🔴 **`AddMarker.jsx` — устаревший help-текст.** Диалог `?` по-прежнему называет маркеры
  `d f h j m s t w z` и советует grep очистки `^[dfhjmstwz]-`, тогда как реальные
  `markLetters` в `ForIndex.jsxinc` — `a…i`: документированный регекс не совпадёт с тем,
  что скрипт реально расставляет. Правильный grep — `^[a-i]-`.
...
- 🟠 **`ProcStoryOrDoс[09.10.2023].jsx`:** `save()` документа внутри цикла по терминам —
  ...
  в имени файла.
- 🟠 **Copy-paste баг очистки массивов** в `ProcNumberLines[3-4,6-8].jsx` и
  `AddLetter.v.2.jsx`: `while (Xid.length > 0) X.pop()` — проверяется длина одного
  массива, а опустошается другой (сейчас безвредно: на старте оба пусты, но это мина).
  Там же в `ProcNumberLines` (~стр. 880) проверка перекрытия «особых диапазонов» панелей
  3–4 использует `nQ == 2` вместо `nQ == 0` — валидация этой пары панелей не выполняется.
- 🟠 **`AddSeeTopic.v.3.v.3.jsx`:** десятки отладочных `$.writeln` в боевом коде; при
  отсутствии `sets/#AddSeeTopic.ini` подставляется английское `"see"` вместо «см.»;
  вставка строк по заранее сохранённым символьным индексам хрупка (обход снизу вверх —
  единственная защита).
- 🟡 **`DeleteUnnecessarySign [SkipTheWord].jsx`:** покраска слова после маркера идёт
  `do…while` до первого чёрного символа **без проверки конца story** — на последнем
  термине материала возможно исключение; цвета сравниваются по ссылке на объект.
